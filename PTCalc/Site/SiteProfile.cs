namespace PTCalc.Site;

/// <summary>
/// PTCalc iki alan adında yayınlanır: <b>ptcalc.tr</b> (varsayılan dil Türkçe) ve
/// <b>ptcalc.net</b> (İngilizce). Eski marka SPPS'in alan adları (spps.tr, spps.tech) kalıcı
...
{
    public const string BrandName = "PTCalc";
    public const string TrHost = "ptcalc.tr";
    public const string EnHost = "ptcalc.net";
    ...
    /// Yerel geliştirmede <c>ptcalc.net.localhost</c> İngilizce varsayılanı verir (Chrome *.localhost'u
    /// 127.0.0.1'e çözer); düz <c>localhost</c> Türkçedir.
    /// </summary>
    public static SiteProfile Resolve(string? host)
    {
        var h = NormalizeHost(host);
        if (h.Contains("ptcalc.net", StringComparison.Ordinal) || h.Contains("spps.tech", StringComparison.Ordinal))
            return En;
        return Tr;
    }
