# EgePharmTech — çalışma kılavuzu

Farmasötik teknoloji hesaplama araçları (Blazor Server, .NET 10). Türkçe **egepharmtech.tr**, İngilizce
**egepharmtech.com**; dil alan adından, kullanıcı seçimi çerezden. Depo herkese açık; sır içermez.
...
- Proje adlarını bu yazımla kullan; `RootNamespace`/`AssemblyName` açıkça `EgePharmTech` (IStringLocalizer kaynak
  adını derleme adından türetir).
- `dotnet run` açıkken `dotnet build`/`publish` çalıştırma; çıktılar çakışır.
- İngilizce varsayılanı yerelde denemek için `http://egepharmtech.com.localhost:5277`.
...
- Arayüz: `@L["Türkçe metin"]`; İngilizcesi `EgePharmTech/Resources/SharedResource.en.resx` (anahtar = Türkçe metin).
- Uzun bloklar: `@if (Lang.IsEn) { <text>…</text> } else { … }`; `<text>` içinde sınır boşluklarını koru, kod bloğunda
  çıplak noktalama bırakma.
- Çekirdek: `EgePharmTech.Core/Resources/CoreText.en.tsv` (satır = anahtar TAB çeviri; csproj'da `WithCulture="false"`).
...
- Site üniversitenin resmi yayını değildir; "Ege" soyadı ve üniversitedir (Hakkında sayfasında yazar).
...
Her yayında `CITATION.cff` (version, date-released) ve `EgePharmTech.Core.csproj` (Version) birlikte güncellenir, `vX.Y.Z`
